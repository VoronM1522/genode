                         ...
                         private Genode::Entrypoint::Io_progress_handler
{
	private:
		...
		void _config_update()
		{
			...
			// Added: honour exit="yes" from config
			if (_config_rom.xml().attribute_value("exit", false)) {
		...
		// 	_config_rom.update();
		// 	_config_rom.xml().with_optional_sub_node("vfs", [&] (Xml_node const &config) {
		// 		_vfs_env->root_dir().apply_config(config); }); // Added -> nstead of . in _vfs_env->root_dir()
		...
		Genode::Constructible<Vfs::Simple_env> _vfs_env { }; // Added


		/* sessions with active jobs */
		Session_queue _active_sessions { };
  		unsigned _num_sessions  { 0 }; // Added
		bool     _exit_requested  { false }; // Added
	...
	protected:
		...
		Session_component *_create_session(const char *args) override
		{
			...
			if (!((session_root == "/")
			 || _vfs_env->root_dir().directory(session_root.base()))) { // Added ->instead of . in _vfs_env.root_dir()
			...
			Session_component *session = new (md_alloc())
				Session_component(_env, label.string(),
				                  Genode::Ram_quota{ram_quota},
				                  Genode::Cap_quota{cap_quota},
				                  tx_buf_size, _vfs_env->root_dir(), // Added _vfs_env.root_dir(),
				                //   _vfs_env.io(),
								  _vfs_env->io(), // Added
		...
		void _destroy_session(Session_component *session) override {
			...
			if (--_num_sessions == 0 && _exit_requested) {
				_vfs_env.destruct();
				_env.parent().exit(0);
			}
		}
	...
	public:
		...
		:
			...
			_env(env)
		{
			_vfs_env.construct(_env, _vfs_heap, vfs_config()); // Added
